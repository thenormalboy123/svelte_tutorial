...
import ts__default, { createSourceFile, ScriptTarget, ScriptKind, isImportDeclaration, isVariableStatement, NodeFlags, isVariableDeclaration, isIdentifier, isFunctionDeclaration, isClassDeclaration, isBlock, isArrowFunction, isExportDeclaration, isNamedExports, isBindingElement, isImportClause, isLabeledStatement, isExpressionStatement, isBinaryExpression, SyntaxKind, forEachChild, isPrefixUnaryExpression, isPostfixUnaryExpression, isParameter, isPropertyAccessExpression, isPropertyAssignment, isObjectBindingPattern, isArrayBindingPattern, isObjectLiteralExpression, getJSDocType } from 'typescript';
    ...
    _serializeElement(node) {
        ...
        if (
            ...
            const childNodesHolder =
                tn === $$5.TEMPLATE && ns === NS$2.HTML ? this.treeAdapter.getTemplateContent(node) : node;
...
function findVerbatimElements(htmlx) {
    ...
    const doc = lib.parseFragment(htmlx, parseOpts);
...
const beforeStart = (start) => start - 1;
const stripDoctype = (str) => {
    const regex = /<!doctype(.+?)>(\n)?/i;
...
function convertHtmlxToJsx(str, ast, onWalk = null, onLeave = null) {
    ...
    const handleActionDirective = (attr, parent) => {
    ...
    const handleAttribute = (attr, parent) => {
    ...
    // {:else} -> </>} else {<>
    const handleElse = (elseBlock, parent) => {
    ...
    const handleComment = (node) => {
        str.remove(node.start, node.end);
    };
    const handleSvelteTag = (node) => {
...
function createEventHandlerTransformer() {
    const events = new Map();
    const handleEventHandler = (node, parent) => {
...
function eventMapToString(events) {
...
function findExortKeyword(node) {
    var _a;
...
function processSvelteTemplate(str) {
    ...
    const handleIdentifier = (node, parent, prop) => {
        if (node.name === '$$props') {
            uses$$props = true;
            return;
        }
        ...
        if (node.name[0] == '$') {
            if (isDeclaration) {
                if (parent.type == 'Property' && prop == 'key')
                    return;
    ...
    const handleScriptTag = (node, parent) => {
        if (parent !== htmlxAst && node.name === 'script') {
            topLevelScripts = topLevelScripts.filter((tag) => tag.start !== node.start || tag.end !== node.end);
        }
    };
...
function processInstanceScriptContent(str, script) {
    const htmlx = str.original;
    ...
    const implicitTopLevelNames = new Map();
    let uses$$props = false;
    let uses$$restProps = false;
    //track if we are in a declaration scope
    let isDeclaration = false;
    ...
    const pushScope = () => (scope = new Scope(scope));
    const popScope = () => (scope = scope.parent);
    const addExport = (name, target = null, type = null, required = false) => {
    ...
    const handleIdentifier = (ident, parent) => {
        ...
        if (isDeclaration || isParameter(parent)) {
            if (!isBindingElement(ident.parent) || ident.parent.name == ident) {
    ...
    const walk = (node, parent) => {
            ...
            if (exportModifier) {
                ...
                if (isLet) {
                    ...
                    node.declarationList.forEachChild((n) => {
                        if (isVariableDeclaration(n) && isIdentifier(n.name)) {
                            addGetter(n.name);
                        }
                    });
                }
        ...
        if (isLabeledStatement(node) &&
            ...
            node.label.text == '$' &&
            node.statement) {
            if (isExpressionStatement(node.statement) &&
                isBinaryExpression(node.statement.expression) &&
                node.statement.expression.operatorToken.kind == SyntaxKind.EqualsToken &&
                isIdentifier(node.statement.expression.left)) {
...
function formatComponentDocumentation(contents) {
    if (!contents)
        return '';
...
function addComponentExport(str, uses$$propsOr$$restProps, strictMode, isTsFile, getters, 
/** A named export allows for TSDoc-compatible docstrings */
className, componentDocumentation) {
    const propDef = 
...
function createRenderFunction({ str, scriptTag, scriptDestination, slots, getters, events, exportedNames, isTsFile, uses$$props, uses$$restProps, }) {
    const htmlx = str.original;
    let propsDecl = '';
    if (uses$$props) {
...
function svelte2tsx(svelte, options) {
    ...
    let { moduleScriptTag, scriptTag, slots, uses$$props, uses$$restProps, events, componentDocumentation, } = processSvelteTemplate(str);
    ...
    const className = (options === null || options === void 0 ? void 0 : options.filename) && classNameFromFilename(options === null || options === void 0 ? void 0 : options.filename);
    addComponentExport(str, uses$$props || uses$$restProps, !!(options === null || options === void 0 ? void 0 : options.strictMode), options === null || options === void 0 ? void 0 : options.isTsFile, getters, className, componentDocumentation);

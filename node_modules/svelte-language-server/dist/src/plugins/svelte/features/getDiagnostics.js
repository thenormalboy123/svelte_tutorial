 ...
 */
async function getDiagnostics(document, svelteDoc) {
 ...
 */
async function tryGetDiagnostics(document, svelteDoc) {
    ...
    try {
        ...
        return (res.stats.warnings || res.warnings || [])
            ...
            .map((diag) => documents_1.mapDiagnosticToOriginal(transpiled, diag));
    }
    catch (err) {
        return (await createParserErrorDiagnostic(err, document)).map((diag) => documents_1.mapDiagnosticToOriginal(transpiled, diag));
    }
}
/**
...
async function createParserErrorDiagnostic(error, document) {
    ...
    if (diagnostic.message.includes('expected')) {
        ...
        const isInScript = documents_1.isInTag(diagnostic.range.start, document.scriptInfo);
